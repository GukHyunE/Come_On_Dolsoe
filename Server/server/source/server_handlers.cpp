...
void handle_robot_status(ServerContext& ctx, ClientInfo* self, const vector<string>& tok) {
    ...
    const string& status = tok[1];
    {
        ...
        cout << "[ROBOT_STATUS] Robot status updated to: " << status << endl;
            ...
            if (status == "AVAILABLE") { // g_kiosk_client 체크 제거
                // 연결된 모든 클라이언트를 순회하며 메시지 전송
                for (ClientInfo* client : ctx.clients) {
                    // 상태 업데이트를 보낸 클라이언트 자신에게는 다시 보내지 않음
                    if (client->fd != self->fd) {
                        send_all_line(client->fd, "ROBOT_STATUS@AVAILABLE");
                        cout << "[ROBOT_STATUS] Sent ROBOT_STATUS@AVAILABLE to client fd=" << client->fd << endl;
                    }
                }
            }    }
