...
{
    QString formatted = QString("%1").arg(track, 3, 10, QChar('0'));
    QString payload = QString("{data: \"%1\"}").arg(formatted);
    QStringList args{"topic", "pub", "/mp3_cmd", "std_msgs/msg/String", payload, "--once"};
    ...
    connect(process, &QProcess::errorOccurred, this, [this, process](QProcess::ProcessError) {
        ...
        qWarning() << "ros2 topic pub failed to start:" << output;
        if (m_currentLanguage == "KR") {
            ui->statusLabel->setText("mp3 명령 전송 실패");
        } else {
            ui->statusLabel->setText("Failed to send mp3 command");
        }
        process->deleteLater();
    });

    connect(process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
            this, [this, process, payload, track](int exitCode, QProcess::ExitStatus status) {
        const QString output = QString::fromUtf8(process->readAll());
        qInfo() << "ros2 topic pub (track" << track << "," << payload << ") -> exit:" << exitCode << "status:" << status << "output:" << output;
    ...
    if (!process->waitForStarted(1000)) {
        qWarning() << "ros2 topic pub process did not start in time for payload:" << payload;
        if (m_currentLanguage == "KR") {
            ui->statusLabel->setText("mp3 명령 전송 실패");
        } else {
            ui->statusLabel->setText("Failed to send mp3 command");
...
void MainWindow::publishServoCommand(quint16 angle)
{
    QString payload = QString("{data: %1}").arg(angle);
    QStringList args{"topic", "pub", "/servo_cmd", "std_msgs/msg/UInt16", payload, "--once"};
    ...
    connect(process, &QProcess::errorOccurred, this, [this, process](QProcess::ProcessError) {
        const QString output = QString::fromUtf8(process->readAll());
        qWarning() << "ros2 topic pub failed to start for servo command:" << output;
        if (m_currentLanguage == "KR") {
            ui->statusLabel->setText("서보 명령 전송 실패");
        } else {
            ui->statusLabel->setText("Failed to send servo command");
        }
        process->deleteLater();
    });

    connect(process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
            this, [this, process, angle](int exitCode, QProcess::ExitStatus status) {
        const QString output = QString::fromUtf8(process->readAll());
        qInfo() << "ros2 topic pub (servo" << angle << ") -> exit:" << exitCode << "status:" << status << "output:" << output;
        if (exitCode != 0 || status != QProcess::NormalExit) {
            if (m_currentLanguage == "KR") {
                ui->statusLabel->setText("서보 명령 전송 실패");
            } else {
    ...
    process->start();
    if (!process->waitForStarted(1000)) {
        qWarning() << "ros2 topic pub process did not start in time for servo angle:" << angle;
        if (m_currentLanguage == "KR") {
            ui->statusLabel->setText("서보 명령 전송 실패");
        } else {
            ui->statusLabel->setText("Failed to send servo command");
