...
MainWindow::MainWindow(QWidget *parent) :
    ...
    m_previousStackIndex(0) // Initialize m_previousStackIndex
{
    ui->setupUi(this);
    ...
    tempPrevButton->hide(); // Temporarily hidden
    ...
    tempNextButton->hide(); // Temporarily hidden
    ...
    ui->titleLabel->setPixmap(titlePixmap.scaled(ui->backgroundLabel->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation)); // Scale to backgroundLabel size

    QPixmap dolsoePixmap(":/images/dolsoe_attention.png");
    ui->dolsoeLabel->setPixmap(dolsoePixmap.scaled(ui->backgroundLabel->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation)); // Scale to backgroundLabel size

    ui->dolsoeLabel->hide();
    ui->wakeUpButton->setStyleSheet("font-size: 36px;");
    ui->wakeUpButton->hide();

    m_titleOpacityEffect = new QGraphicsOpacityEffect(ui->titleLabel);
    m_titleOpacityEffect->setOpacity(0.0); // Set initial opacity to 0
    ...
    m_learningPage->setObjectName("learningPage"); // Name the widget for the stylesheet
    // Use a stylesheet to set the background image. This allows other widgets on top.
    // The image will be centered and not scaled.
    m_learningPage->setStyleSheet(
    ...
    // Use a layout to position the text label at the bottom
    QVBoxLayout *learningLayout = new QVBoxLayout(m_learningPage);
    learningLayout->addStretch(); // Pushes the following widgets to the bottom

    m_learningStatusLabel = new QLabel(tr("돌쇠가 학습 중입니다. 잠시만 기다려 주세요!"));
    m_learningStatusLabel->setAlignment(Qt::AlignCenter);
    m_learningStatusLabel->setStyleSheet("background-color: transparent; color: black; font-size: 36px;");
    learningLayout->addWidget(m_learningStatusLabel);

    // --- MANUAL POSITIONING ---
    // To adjust the text's vertical position, change the bottom margin.
    // A larger value moves the text UP, a smaller value moves it DOWN.
    int bottomMargin = 450; // Distance from the bottom edge in pixels
    learningLayout->setContentsMargins(0, 0, 0, bottomMargin);
    ...
    letsgoLayout->addWidget(ui->carryingLabel); // Image added first
    letsgoLayout->addWidget(ui->pushButton_3); // Button added second
    ...
    // Set font size and alignment for finishLabel
    ui->finishLabel->setStyleSheet("font-size: 36px;");
    ui->finishLabel->setAlignment(Qt::AlignCenter);
    ...
    connect(m_connectionRetryTimer, &QTimer::timeout, this, &MainWindow::initializeConnection);
    connect(socket, &QTcpSocket::connected, this, &MainWindow::queryInitialRobotStatus);

    // Start connection attempts immediately and then every 5 seconds
    initializeConnection();
    m_connectionRetryTimer->start(5000); // Retry every 5 seconds
...
bool MainWindow::eventFilter(QObject *obj, QEvent *event)
{
    if (event->type() == QEvent::MouseButtonPress) {
        if (obj == ui->nameLE) {
            ...
            // Calculate global position below verticalLayoutWidget_2 and centered
            QPoint globalBottomLeft = ui->verticalLayoutWidget_2->mapToGlobal(QPoint(0, ui->verticalLayoutWidget_2->height()));
            int x_pos_keyboard = globalBottomLeft.x() + (ui->verticalLayoutWidget_2->width() - keyboard->width()) / 2;
            int y_pos_keyboard = globalBottomLeft.y() + 10; // 10px padding
            keyboard->move(x_pos_keyboard, y_pos_keyboard);
            keyboard->show();
        } else if (obj == ui->numberLE) {
            numpad->setLineEdit(static_cast<QLineEdit*>(obj), false);
            // Calculate global position below verticalLayoutWidget_2 and centered
            QPoint globalBottomLeft = ui->verticalLayoutWidget_2->mapToGlobal(QPoint(0, ui->verticalLayoutWidget_2->height()));
            int x_pos_numpad = globalBottomLeft.x() + (ui->verticalLayoutWidget_2->width() - numpad->width()) / 2;
            int y_pos_numpad = globalBottomLeft.y() + 10; // 10px padding
            numpad->move(x_pos_numpad, y_pos_numpad);
            numpad->show();
...
void MainWindow::showDolsoeAndButton()
{
    ...
    // ui->wakeUpButton->setStyleSheet("font-size: 36px;"); // Moved to constructor

    // Ensure wakeUpButton is correctly sized and centered before its animation starts
    ui->wakeUpButton->adjustSize();
    int centeredX = (ui->startPage->width() - ui->wakeUpButton->width()) / 2;
    int targetY = 1350; // Consistent Y position
    ui->wakeUpButton->move(centeredX, targetY);
...
void MainWindow::onSocketReadyRead()
{
    ...
    } else if (response == "ROBOT_AVAILABLE") {
        ui->stack->setCurrentIndex(0); // Navigate to startPage
    } else if (response.startsWith("ROBOT_STATUS@")) {
        QStringList parts = response.split('@');
        if (parts.size() > 1) {
            QString status = parts.at(1);
            if (status == "IDLE" || status == "AVAILABLE") {
                ui->stack->setCurrentIndex(0); // Robot is available, go to start page
            } else {
                ui->stack->setCurrentWidget(ui->noRobotPage); // Robot not available, go to no robot page
            }
        }
    }
}
...
{
    qDebug() << "Learning complete signal received. Navigating to letsgo page.";
    ui->stack->setCurrentWidget(ui->letsgoPage);
}
#endif
...
void MainWindow::on_pushButton_3_clicked()
{
    ...
    if (socket->state() == QAbstractSocket::ConnectedState) {
        ...
        qDebug() << "Sent to server:" << message_to_server;
            ...
            // Send command to server to trigger robot action
            QString robot_action_message = "ROBOT_COMMAND@AUDIO_001_SERVO_ACTION";
            socket->write(robot_action_message.toUtf8() + "\n");
            socket->flush();
            qDebug() << "Sent to server:" << robot_action_message;

            // Inform server that robot is in use
            QString status_message = "ROBOT_STATUS@IN_USE";
            socket->write(status_message.toUtf8() + "\n");
            socket->flush();
            qDebug() << "Sent to server:" << status_message;
        } else {
            qWarning() << "Socket not connected. Cannot send password request or robot action or status to server.";
        }

        // Navigate to the noRobotPage
        ui->stack->setCurrentWidget(ui->finishPage);}
...
void MainWindow::onStackPageChanged(int index)
{
    ...
    } else if (index == ui->stack->indexOf(ui->noRobotPage)) {
        QMovie *movie = ui->namuLabel->movie();
        if (movie) {
            QTimer::singleShot(0, [this, movie]() {
                const QPixmap currentPixmap = movie->currentPixmap();
                if (!currentPixmap.isNull()) {
                    int labelWidth = ui->namuLabel->width();
                    QPixmap scaledPixmap = currentPixmap.scaledToWidth(labelWidth, Qt::SmoothTransformation);
                    movie->setScaledSize(scaledPixmap.size());
                    ui->namuLabel->setFixedHeight(scaledPixmap.height());
                }
            });
        }
    }
...
void MainWindow::on_pushButton_clicked()
{
    ...
    QTimer::singleShot(1500, this, [this]() {
        ...
        m_stateCountdown = 5;
                    // ui->cameraLabel->setText(tr("%1초 뒤에 촬영을 시작합니다.").arg(m_stateCountdown));        m_sequenceTimer->start(1000);
    });
}
...
void MainWindow::on_koreanButton_clicked()
{
    ...
    connect(fadeOutGroup, &QParallelAnimationGroup::finished, this, [this, fadeOutGroup, dolsoeEffect, buttonEffect]() {
        ...
        // Adjust button size and reposition after text change
        ui->wakeUpButton->adjustSize();
        // Assuming startPage has a width of 1080 (from mainwindow.ui)
        int newX = (ui->startPage->width() - ui->wakeUpButton->width()) / 2;
        int newY = 1350; // Maintain the new Y position
        ui->wakeUpButton->move(newX, newY);
...
void MainWindow::changeLanguage(const QString &language)
{
    ...
    // Explicitly retranslate the learning status label
    m_learningStatusLabel->setText(tr("돌쇠가 학습중입니다. 잠시만 기다려 주세요!"));
...
{
    qDebug() << "Button pressed signal received (Shooting complete). Navigating to learning page.";
    ui->stack->setCurrentWidget(m_learningPage);
}
#endif
...
{
    qDebug() << "Finish page timer timed out. Querying robot status.";
    if (socket->state() == QAbstractSocket::ConnectedState) {
        QString query_message = "QUERY_ROBOT_STATUS";
        socket->write(query_message.toUtf8() + "\n");
        socket->flush();
        qDebug() << "Sent to server:" << query_message;
    } else {
        qWarning() << "Socket not connected. Cannot query robot status after finish page timeout.";
        // If not connected, assume not available and show noRobotPage
        ui->stack->setCurrentWidget(ui->noRobotPage);
    }
}
...
void MainWindow::initializeConnection()
{
    if (socket->state() == QAbstractSocket::UnconnectedState) {
        ...
        QString serverAddress = "10.10.16.63"; // Or get from a config file
        quint16 serverPort = 5000;
        socket->connectToHost(serverAddress, serverPort);
    }
}

void MainWindow::queryInitialRobotStatus()
{
    qDebug() << "Successfully connected to server. Querying initial robot status.";
    // Stop retrying once connected
    m_connectionRetryTimer->stop();

    // Ask the server for the robot's current status
    QString query_message = "QUERY_ROBOT_STATUS";
    socket->write(query_message.toUtf8() + "\n");
    socket->flush();
}
